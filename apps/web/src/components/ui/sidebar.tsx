...
const SidebarProvider = ({
  ...
  const setOpen = useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // biome-ignore lint/suspicious/noDocumentCookie: IntentUI persists only the non-sensitive sidebar preference.
      ...
      className={composeRenderProps(
        ...
        (className, { isFocusVisible, isPressed, isHovered, isDisabled }) =>
          twMerge(
            ...
            "[--sidebar-current-bg:var(--color-sidebar-primary)] [--sidebar-current-fg:var(--color-sidebar-primary-foreground)]",
            isCurrent &&
              "bg-(--sidebar-current-bg) font-medium text-(--sidebar-current-fg) hover:bg-(--sidebar-current-bg) hover:text-(--sidebar-current-fg) [&_.text-muted-foreground]:text-foreground/80 [&_svg:not([class*='text-'])]:text-(--sidebar-current-fg) hover:[&_svg:not([class*='text-'])]:text-(--sidebar-current-fg)",
            isFocusVisible &&
...
const SidebarTreeItem = <T extends object>({
  className,
  ...props
}: TreeItemProps<T>) => {
  return (
