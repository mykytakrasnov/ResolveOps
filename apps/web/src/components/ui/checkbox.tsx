...
export function CheckboxGroup({ className, ...props }: CheckboxGroupProps) {
  return (
    <CheckboxGroupPrimitive
      ...
      data-slot="control"
      className={cx(
        "flex flex-col gap-3 has-[[slot=description]]:not-has-[[slot=errorMessage]]:gap-6 has-[[slot=description]]:**:data-[slot=label]:font-medium **:[[slot=description]]:block",
        className,
      )}
    />
  );
            ...
            >
              <span
                ...
                className={twMerge([
                  ...
                  isInvalid &&
                    "inset-ring-destructive/70 bg-destructive/5 text-destructive ring-destructive/20 group-hover:inset-ring-destructive/70",
                ])}
              >
                {indicator}
              </span>
