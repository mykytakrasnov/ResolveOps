...
const DialogHeader = ({ className, ...props }: DialogHeaderProps) => {
  return (
    <div
      ...
      className={twMerge(
        "relative flex flex-col gap-1 p-(--gutter) pb-[calc(var(--gutter)---spacing(3))]",
        className,
      )}
    >
...
const DialogClose = ({ variant = "ghost", ref, ...props }: ButtonProps) => {
  return <Button slot="close" ref={ref} variant={variant} {...props} />;
};

interface CloseButtonIndicatorProps
  extends Omit<PrimitiveButtonProps, "children"> {
  className?: string;
